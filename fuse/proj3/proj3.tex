...
\begin{itemize}
  ...
  \begin{itemize}
    \item \textit{LFS ``superblock''} (as \colorbox{shade}{\textbf{\ttfamily struct superblock}}) keeps basic properties of the file system. In our project these are all constants, so we only write them once, and do not directly read them.
...
\texttt{atime} is earlier than \texttt{mtime} or \textbf{ctime}, or
...
obtain an opportunity to work, although it prints debug messages. }
  ...
  \item \texttt{ENABLE\_PERMISSION} is a flag for
...
can not even delete files. This will be settled in Project 4.
