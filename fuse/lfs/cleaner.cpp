...
void gc_dd_segbuf_summary(int block_index, int _i_number, int _direct_index) {
...
void gc_move_to_segment() {
    if (gc_cur_block == DATA_BLOCKS_IN_SEGMENT-1 || gc_next_imap_index == DATA_BLOCKS_IN_SEGMENT) {    // Segment buffer is full, and should be flushed to disk file.
        ...
        if (next_segment == -1) {
            ...
            /* TBD: copy the disk file as a back-up, and perform thorough gc. */
            exit(-1);
        } else {
...
int gc_new_data_block(void* data, int _i_number, int _direct_index) {
...
void collect_garbage() {
    ...
    block data;
    if (utilization[0].count == 0) && (!clean_thoroughly) {
        ...
        for (int i=0; i<CLEAN_NUM; i++) {
            ...
            idx++;    // Proceed to the next lowly-utilized segment.
        }
